/**
 * Pangolinfo MCP - tool: google_ai_search
 *
...
export const googleAiSearch: Tool<typeof inputSchema> = {
  name: "google_ai_search",
  description: t({
    zh: `[Google SERP + AI Overview 抓取] 抓 Google 搜索结果，含顶部 AI Overview 摘要、organic 自然位、相关搜索词。两种模式：overview（标准 SERP）/ ai_mode（沉浸式对话，支持多轮追问）。
...
Don't use: 想在 Amazon 站内搜（用 search_amazon）；只要趋势曲线（用 google_trends，更便宜更聚焦）。
...
Tips: 单次查询用 overview 更经济；只有要"拆解复杂问题 + 连续追问"时才上 ai_mode。followups 超 5 条响应明显变慢。`,
    en: `[Google SERP + AI Overview scrape] Scrape Google search with top AI Overview, organic results, and related searches. Two modes: overview (standard SERP) / ai_mode (immersive multi-turn conversational search).
...
Don't use: for on-Amazon search (use search_amazon); when only the trend curve matters (use google_trends — cheaper and tighter).
Returns: data.{ results_num, ai_overview, json.items[ { type:'ai_overview', items:[{content:[...], references:[{title,url,domain}]}] }, { type:'organic', items:[{title,url,text}] }, { type:'related_searches', items:[...] } ], screenshot, taskId }.
...
Tips: prefer overview for single queries (cheaper); use ai_mode only when you need decomposed multi-turn investigation. Followups > 5 visibly slow down responses.`,
  ...
  async execute(input, ctx) {
    ...
    ctx.logger.info(
      `google_ai_search: mode=${input.mode} query="${input.query}" followups=${input.followups?.length ?? 0}`,
    );
