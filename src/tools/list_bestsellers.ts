...
const inputSchema = z.object({
  ...
  format: z
    ...
    .describe(
      t({
        zh: "返回格式。默认 'json'——结构化 Top-50 ASIN 排名列表。需要原始页面阅读时切 'markdown'。",
        en: "Response format. Defaults to 'json' — structured Top-50 ranked ASIN list. Use 'markdown' for the rendered page text.",
      }),
    ),
});
...
export const listBestsellers: Tool<typeof inputSchema> = {
  ...
  description: t({
    zh: `[Amazon 热销榜] 拉某类目的 Best Sellers Top-50，含 24h 排名变化。
...
Tips: categorySlug 是 Amazon URL 路径里的英文短横线串，从 amazon.com/Best-Sellers 顶部导航能看到。`,
    en: `[Amazon Best Sellers] Top-50 ranking for a category with 24h rank deltas.
