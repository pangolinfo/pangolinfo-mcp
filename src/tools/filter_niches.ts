 ...
 *  - Pagination size is capped at 10 by the backend; default 3 to avoid token bloat.
 *
...
const inputSchema = z.object({
  marketplaceId: z
    ...
    .describe(
      t({
        zh: "Amazon 站点 ID（必填）。⚠️ 后端目前仅支持 US；传其他站点会失败或回退。默认且只用 US。",
        en: "Amazon marketplace id (required). ⚠️ Backend currently supports US only; other marketplaces will fail or fall back. Use US (the default).",
      }),
    ),
  ...
  size: z
    ...
    .describe(
      t({
        zh: "每页条数,上限 10(服务端硬限制),默认 3(小默认值避免 AI 上下文超限 — 需要更宽扫描时显式传 size=10)。",
        en: "Page size, max 10 (backend hard limit), default 3 (small default to keep responses under AI context limits — pass size=10 explicitly when you need a wider sweep).",
      }),
    ),
  sortField: z
    .string()
...
Cost: ~1 积点/次, ~5s。
Tips: size 上限 10(默认 3)；50+ 长尾筛选字段走 extraFilters 透传；典型蓝海过滤组合 = searchVolumeT90Min 高 + top5ProductsClickShareT360Max 低 + productCountMax 中 + searchVolumeGrowthT90Min > 0 + returnRateT360Max ≤ 0.10（低退货）。退货率筛选用 returnRateT360Max（上限，0-1 小数），返回里 returnRateT360 字段直接给出具体退货率。`,
