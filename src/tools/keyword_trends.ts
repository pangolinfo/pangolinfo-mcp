/**
 * Pangolinfo MCP - tool: keyword_trends
 *
...
export const keywordTrends: Tool<typeof inputSchema> = {
  name: "keyword_trends",
  description: t({
    zh: `[Keyword Trends via Google Trends] 关键词热度趋势(数据来源:Google Trends,使用须遵守 Google 服务条款)。 时间序列 + 地区热度 + 相关上升查询（含 Breakout 标记）。一次最多 5 个关键词同图对比。
...
Tips: timeRange = today 12-m (默认) | today 3-m | today 5-y | all 等；region = ISO 国家码或 'WORLD'；language 影响相关查询的语言。`,
    en: `[Keyword Trends via Google Trends] Keyword popularity (data source: Google Trends; use must comply with Google Terms of Service). Time series + per-region heatmap + rising related queries (with 'Breakout' tags). Compare up to 5 keywords on one chart.
...
Tips: timeRange = today 12-m (default) | today 3-m | today 5-y | all ; region = ISO country code or 'WORLD'; language affects related-query language.`,
  ...
  async execute(input, ctx) {
    ctx.logger.info(
      `keyword_trends: keywords=[${input.keywords.join(",")}] timeRange=${input.timeRange} region=${input.region}`,
    );
