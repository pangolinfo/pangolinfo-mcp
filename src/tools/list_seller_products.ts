...
Returns: data.json[0].data.{ pageIndex, maxPage, nextPage, results[{ asin, title, price, star, rating, rank, img }] } —— 每页 24 条。**翻页两种方式**: ① page 定位看某一页（默认 1）；② pageCount 一次累计前 N 页合并（N≤3，结果扁平进同一 results）。pageCount>1 时 pageIndex/nextPage 等会置空（多页已合并）。**类目筛选**: categoryId 按类目过滤店铺商品。
...
Tips: 要一次性拿全量多页 SKU 用 pageCount（最多 3 页）；只看具体某一页用 page；单纯看一下店铺有什么货首页就够。Amazon 自营 sellerId = 'ATVPDKIKX0DER'。`,
...
Returns: data.json[0].data.{ pageIndex, maxPage, nextPage, results[{ asin, title, price, star, rating, rank, img }] } — 24 rows/page. **Two pagination modes**: ① page locates a specific page (default 1); ② pageCount accumulates the first N pages in one call (N≤3, flat-merged into the same results). When pageCount>1, pageIndex/nextPage are blanked (pages already merged). **Category filter**: categoryId filters the seller's products by category.
...
Tips: use pageCount to grab the full multi-page SKU set in one shot (max 3 pages); use page to view one specific page; the first page is enough to glance at what the store sells. Amazon first-party sellerId = 'ATVPDKIKX0DER'.`,
  }),
  inputSchema,
