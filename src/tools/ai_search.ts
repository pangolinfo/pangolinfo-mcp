 ...
 *     + `organic` + `related_searches` + `sponsered` (Google ads:
 *     items[] each {type:'result', url, position:'top'|'bottom',
 *     title_of_page, title_above_url}). Synchronous, ~30s, 2 points/call.
...
Returns: data.{ results_num, ai_overview, json.items[ { type:'ai_overview', items:[{content:[...], references:[{title,url,domain}]}] }, { type:'organic', items:[{title,url,text}] }, { type:'related_searches', items:[...] }, { type:'sponsered', items:[{type:'result', url, position:'top'|'bottom', title_of_page, title_above_url}] } ], screenshot, taskId }。⚠️ 广告块上游 type 拼写就是 'sponsered'（少一个 o，非笔误，按原样匹配，别用 'sponsored' 去找）——含 Google 广告（购物 + 文字）的落地页 url、标题(title_of_page)、显示品牌域名(title_above_url)。**position** 标记该广告出现在页面顶部('top')还是底部('bottom')，顶部广告曝光权重更高。
Pair with: ↑ query 由用户提问推导；mode='ai_mode' 时传 followups[1..5] 做多轮追问；↓ ai_overview.references[].url 可作外部权威源，organic 结果可喂下游做内容竞争分析，sponsered[].url + title_above_url 可看竞品付费投放的落地页与品牌，按 position 区分头部/尾部广告位。
